...
<div class="container">
  ...
  <thead>
    <tr>
      ...
      <th style="width: 40%;">File Preview</th>
...
Your browser does not support the video tag.
      ...
      <td>{{$users_id->file_name}}</td>
      <td>{{$users_id->activity_name}}</td>
      <td>{{$users_id->activity_title}}</td>
      <td>{{$users_id->activity_description}}</td>
      <td><a href="{{route('showcase.edit', $users_id->id)}}"><button type="button" class="btn btn-outline-primary float-left edit">Edit</button></a>
      <a href="{{route('showcase.destroy', $users_id->id)}}"><button type="button" class="btn btn-outline-danger float-right delete">Delete</button></a></td>
    </tr>
    @endforeach
